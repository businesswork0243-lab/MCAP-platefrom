...
brandRouter.put('/:id', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  await query(
    `UPDATE brand_profiles SET
      name = COALESCE($1, name), mission = COALESCE($2, mission), vision = COALESCE($3, vision),
      positioning = COALESCE($4, positioning), is_default = COALESCE($5, is_default), updated_at = NOW()
     WHERE id = $6`,
    [d.name, d.mission, d.vision, d.positioning, d.isDefault, req.params.id]
  )
